...
export function* fetchCategoriesAsync() {
  ...
  try {
    const categoriesArray = yield call(getCategoriesAndDocuments, "categories"); //* the categoriesArray fetches the categories collection reference. yield instead of await, and the call method is used to invoke the function explicitly
    yield put(fetchCategoriesSuccess(categoriesArray)); //*if the fetching succeeded, categories = categoriesArray
  } catch (error) {
    yield put(fetchCategoriesFailed(error)); //* If failed, payload = error, or in english, return error. put is equivalent to dispatch in redux-saga
  }
}
